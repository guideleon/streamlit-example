...
st.markdown('<p style="font-size:20px; color:#ff6600;">Temperature: 56 degrees</p>', unsafe_allow_html=True)
st.markdown('<p style="font-size:20px; color:#ff6600;">Fuel Price: 3.1 dollars per gallon</p>', unsafe_allow_html=True)
st.markdown('<p style="font-size:20px; color:#ff6600;">CPI: 308.8 base points</p>', unsafe_allow_html=True)
st.markdown('<p style="font-size:20px; color:#ff6600;">Unemployment: 3.7%</p>', unsafe_allow_html=True)
...
Temperature = st.slider("Temperature", min_value=0, max_value=100, value=56)
Fuel_Price = st.number_input("Fuel Price", value=3.1)
CPI = st.number_input("CPI(consumer price index", value=308.8)
Unemployment = st.number_input("Unemployment Rate", value=3.7)
...
if st.button("Let´s Predict"):
    ...
    st.markdown(f"The sales for the specified conditions will be: <span style='font-weight:bold;color:red;'>${Prediccion_str_con_separador}</span> dollars", unsafe_allow_html=True)
